...
public class CRC16Util {
	...
	/**
	 * 获取字符串（10进制）的16进制字节数组
	 * @param str
	 * @return
	 */
	public static byte[] getStrToHex(String str){
	...
	public static String getCrc(String tem) {
		...
		byte[] sbuf = CRC16Util.getSendBuf(tem);
		System.out.println("这是得到的crc" + CRC16Util.getBufHexStr(sbuf));
		return CRC16Util.getBufHexStr(sbuf).substring(12, 16).toLowerCase();
	}
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		String[] totalTemp = new String[10];// 38
		int i = 0;

		totalTemp[i++] = "02";// modbus地址
		totalTemp[i++] = "0F";// 批量写输出线圈的功能码
		totalTemp[i++] = "00";
		totalTemp[i++] = "00";// 输出线圈的起始地址是0000
		totalTemp[i++] = "00";
		totalTemp[i++] = "11";// 要写的线圈数0011,表示写24个,必须是8的倍数
		totalTemp[i++] = "03";// 表示要写几个值 是4个十六进制数代表
		totalTemp[i++] = "ff";
		totalTemp[i++] = "fe";
		totalTemp[i++] = "7f";// 写的数据CRC会自动跟在后面
		// 将字符串数组转化为字符串
		StringBuffer sb = new StringBuffer();
		for (int j = 0; j < totalTemp.length; j++) {
			sb.append(totalTemp[j]);
		}
		String str = sb.toString(); // 转化结束

		// 获取要发送的byte数组,这个是直接发送的
		byte[] sendBuf = CRC16Util.getSendBuf(str);
		// 为了验证方便,将16进制的byte数组转化为字符串输出
		System.out.println(CRC16Util.getBufHexStr(sendBuf));
	}
}
